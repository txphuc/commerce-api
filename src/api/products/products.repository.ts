...
export class ProductsRepository extends Repository<Product> {
  ...
  async getAllProductsPagination(
    ...
    const entities = rawEntities.map((entity) => {
      ...
      info.productId = Number(entity.info_product_id);
      info.minPrice = Number(entity.info_min_price);
      info.maxPrice = Number(entity.info_max_price);
      info.minDiscount = Number(entity.info_min_discount);
      info.maxDiscount = Number(entity.info_max_discount);
      info.totalQuantity = Number(entity.info_total_quantity);
      info.totalPurchased = Number(entity.info_total_purchased);
      product.info = info;
      return product;
    });
